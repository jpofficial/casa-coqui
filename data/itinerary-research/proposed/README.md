...
Scout agents write daily activity proposals here. Julio reviews them, flips `approved: true` on the ones to keep, then runs `node scripts/seed/merge-itinerary-proposals.js` to integrate approved entries into the live DynamoDB activities table.
...
4. **Failure logging**: if you complete with 0 proposals (rate-limited, no good sources, etc.), write a log to `data/itinerary-research/failed/<date>-<scout>.json` explaining why. Silent zero-runs are bugs.
...
`data/itinerary-research/failed/` holds error logs from scout runs that completed with zero output (rate-limit, no candidates found, etc.). Check here when a scout appears silent.
