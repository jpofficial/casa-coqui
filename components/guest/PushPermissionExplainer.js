    ...
    return (
      <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl px-3 py-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-green-900 leading-snug">{t(locale, 'push_enableCompact')}</p>
          <p className="text-xs text-green-700">{t(locale, 'push_enableCompactDesc')}</p>
        </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="bg-green-600 px-5 pt-6 pb-8 text-white text-center">
        ...
        <h2 className="text-lg font-bold">{t(locale, 'push_stayInTheLoop')}</h2>
        <p className="text-sm text-green-100 mt-1">
          {t(locale, 'push_explainerDesc')}
        </p>
      </div>
      ...
      <div className="px-5 py-4 flex flex-col gap-2.5">
        {BENEFITS.map((b) => (
          <div key={b.labelKey} className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-700">{t(locale, b.labelKey)}</span>
          </div>
        ))}
      </div>
      ...
      <div className="px-5 py-5 flex flex-col gap-2">
        <button
          ...
          className="w-full py-3.5 rounded-xl bg-green-600 text-white font-semibold text-sm
            hover:bg-green-700 active:bg-green-800 transition disabled:opacity-60"
        >
          {enabling ? t(locale, 'push_requestingPermission') : t(locale, 'push_enableNotificationsBtn')}
        </button>
        {onDismiss && (
          <button
            onClick={onDismiss}
