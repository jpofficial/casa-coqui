...
function AdminLayoutInner({ children }) {
  ...
  if (role === 'cleaner' || role === 'maintenance') {
    ...
    return (
      <div className="min-h-screen bg-cafe-50 flex flex-col">
        {/* Foreground toast for staff notifications */}
        ...
        <header className="bg-white/80 backdrop-blur-sm border-b border-cafe-200 px-4 py-3.5 flex items-center justify-between sticky top-0 z-40 shadow-brand">
          ...
          <div className="flex items-center gap-1">
            {process.env.NEXT_PUBLIC_HOST_WHATSAPP && (
              <a
                href={`https://wa.me/${process.env.NEXT_PUBLIC_HOST_WHATSAPP}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center w-9 h-9 rounded-full bg-[#25D366] text-white
                  hover:bg-[#20bd5a] active:bg-[#1da851] transition-colors min-h-[44px] min-w-[44px]"
                aria-label={t(locale, 'contactHost')}
              >
  ...
  const allMoreLinks = [
    ...
    { href: '/admin/pricing', label: t(locale, 'admin_nav_pricing') },
