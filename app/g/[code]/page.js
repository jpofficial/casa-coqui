...
function GuestHomeContent({ params }) {
  ...
  return (
    <div className="px-4 pt-5 pb-4">
      ...
      {/* ── 3. Check-in prompt (optional, subtle) ─────────────────────── */}
      {!loading && !membersLoading && !hasCheckedIn && (
        <section className="mb-4">
          <Link href={`/g/${code}/checkin`}>
            <div className="bg-white border border-gray-200 rounded-xl px-4 py-3 flex items-center gap-3 hover:bg-gray-50 active:scale-[0.99] transition-all">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.75} stroke="currentColor" className="w-5 h-5 text-coqui-500 flex-shrink-0">
              ...
              <div className="min-w-0">
                <p className="text-sm font-medium text-gray-800">{t(locale, 'completeCheckIn')}</p>
                <p className="text-xs text-gray-400 mt-0.5">{t(locale, 'checkinOptionalHint')}</p>
              </div>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4 text-gray-300 ml-auto flex-shrink-0">
                <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
              </svg>
            </div>
          </Link>
      ...
      <section className="mb-5">
        ...
        {loading ? <GridSkeleton /> : (
          <div className="grid grid-cols-2 gap-3">
            {/* Check-In Guide — always goes to access page */}
            <NavCard
              href={`/g/${code}/access`}
