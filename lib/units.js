...
export function getUnits(settings) {
  if (settings?.units && Array.isArray(settings.units) && settings.units.length > 0) {
    // Normalize every entry to a clean {id, name} object.
    // Handles: plain strings (legacy seed), objects without id (mangled spread),
    // and correct {id, name} objects.
    return settings.units.map((entry, i) => {
      const fallbackId = DEFAULT_UNITS[i]?.id || `unit-${String.fromCharCode(97 + i)}`;
      const fallbackName = DEFAULT_UNITS[i]?.name || `Unit ${String.fromCharCode(65 + i)}`;

      if (typeof entry === 'string') {
        return { id: fallbackId, name: entry };
      }
      if (typeof entry === 'object' && entry !== null) {
        return { id: entry.id || fallbackId, name: entry.name || fallbackName };
      }
...
export function resolveUnitWifi(booking, settings) {
  ...
  // 1b. Fallback: resolve unitId from booking.unit display name (legacy bookings)
  if (!booking?.unitId && booking?.unit && Array.isArray(settings?.unitWifi)) {
    const units = getUnits(settings);
    const rawUnit = booking.unit;

    // Try exact name match → get id
    let resolved = units.find((u) => u.name === rawUnit);

    // Try normalized match (case-insensitive, strip "Unit " prefix)
    if (!resolved) {
      const normalized = rawUnit.replace(/^Unit\s*/i, '').trim().toLowerCase();
      resolved = units.find((u) => {
        const nameNorm = u.name.replace(/^Unit\s*/i, '').trim().toLowerCase();
        return nameNorm === normalized || u.name.toLowerCase() === rawUnit.toLowerCase();
      });
    }

    // Try DEFAULT_UNITS positional mapping
    if (!resolved) {
      const defaultIdx = DEFAULT_UNITS.findIndex(
        (d) => d.name.toLowerCase() === rawUnit.toLowerCase()
      );
      if (defaultIdx >= 0 && defaultIdx < units.length) {
        resolved = units[defaultIdx];
      }
    }

    if (resolved) {
      const match = settings.unitWifi.find((w) => w.unitId === resolved.id);
      if (match?.ssid) {
        return { ssid: match.ssid, password: match.password || null };
      }
    }
