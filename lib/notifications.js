...
export async function sendNotification({ to, title, body, data = {} }) {
  try {
    // Data-only message — no `notification` key. This prevents the FCM SDK
    // from auto-displaying a notification in the background, which would
    // duplicate the one shown by our onBackgroundMessage handler in the SW.
    const enrichedData = { ...data };
    if (title) enrichedData.title = String(title);
    if (body) enrichedData.body = String(body);
...
//
// @param {object} opts
//   title     {string}    Notification title (used for Firestore doc & fallback)
//   body      {string}    Notification body  (used for Firestore doc & fallback)
//   localizer {function}  Optional (locale) => ({ title, body }) for per-guest
//                          push localization. When provided, FCM push content is
//                          localized per recipient. The Firestore doc always
//                          stores the English title/body for admin readability.
//   data      {object}    Optional FCM data payload
//
// @returns {{ success: boolean, data?: { total, push, failed }, error?: string }}
// ---------------------------------------------------------------------------
export async function broadcastToActiveGuests({ title, body, type = 'general', data = {}, localizer }) {
  try {
    ...
    for (const code of bookingCodes) {
      ...
      // Resolve per-guest locale for push content.
      let pushTitle = title;
      let pushBody = body;
      if (localizer) {
        const guestLocale = prefsDoc.exists ? (prefsDoc.data().locale || 'en') : 'en';
        const localized = localizer(guestLocale === 'es' ? 'es' : 'en');
        pushTitle = localized.title;
        pushBody = localized.body;
      }

      // FCM data values must be strings. Title and body are included in the
      // data payload (not the notification key) so the SW's onBackgroundMessage
      // handler is the sole notification displayer — prevents FCM SDK auto-show
      // from creating duplicates.
      const enrichedData = {};
      for (const [k, v] of Object.entries({ ...data, type, bookingCode: code })) {
        if (v != null) enrichedData[k] = String(v);
      }
      enrichedData.title = String(pushTitle);
      enrichedData.body = String(pushBody);
  ...
  try {
    ...
    if (!tokenSnap.empty) {
      ...
      try {
        await messaging.send({
          token,
          data: { title: String(title), body: String(body), type: category, bookingCode },
        });
        method = 'push';
      } catch (fcmError) {
...
export async function sendPushOnly({ bookingCode, title, body, category = 'laundry', data = {} }) {
  ...
  try {
    ...
    if (!tokenSnap.empty) {
      ...
      try {
        const pushData = { ...data };
        pushData.title = String(title);
        pushData.body = String(body);
        await messaging.send({
          token,
          data: pushData,
        });
