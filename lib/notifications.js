...
export async function broadcastToActiveGuests({ title, body, type = 'general', data = {} }) {
  try {
    ...
    // 3. Check notification preferences — exclude guests who opted out of this category.
    const fcmTokens = [];
...
// ---------------------------------------------------------------------------
export async function sendDirectMessage({ bookingCode, title, body, category = 'general' }) {
  try {
