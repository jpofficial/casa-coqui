...
/** Model used for all pricing AI calls (recommendation, chat, evidence). */
...
// ---------------------------------------------------------------------------
// Evidence question answering — separate from recommendation explanation
// ---------------------------------------------------------------------------

const EVIDENCE_SYSTEM_PROMPT = `You are a pricing data analyst for a short-term rental property in San Juan, Puerto Rico. You answer questions about historical pricing data, competitor behavior, availability patterns, and market trends.

YOUR RULES:
1. Answer ONLY from the data provided. Do not speculate beyond the evidence.
2. Cite specific numbers: competitor counts, percentages, dollar amounts, dates.
3. When the data source is "calendar" (from calendar_availability), use "booked" for unavailable dates — this represents real calendar booking data. When the data source is "observations", use "unavailable" since it only captures listing-level status.
4. Do not name individual competitors — refer to them by count or rank only. EXCEPTION: when asked to list competitors or their links, you may use the competitor name.
5. Maximum 200 words.
6. Be direct and factual. The host wants clear answers, not hedging.
7. If the data is insufficient to answer, say so explicitly and suggest what data is needed.
8. All prices in USD.
9. Always state the scope of your answer: which unit(s) and time window the data covers.
   The evidence payload includes unitScope and dateScope — use them.
10. If data covers both units, present each unit separately when the numbers differ meaningfully.
11. When competitor entries include a "url" field, ALWAYS include these Airbnb links in your answer so the host can view the listings directly.
12. When evidence includes "bookedPct" or "bookedDates", present these as actual booking data, not estimates. Calendar data captures the real state of each competitor's Airbnb calendar.
13. Format booking data clearly: "3 competitors are 50%+ booked in April" is better than a wall of numbers. Lead with the insight, then provide details.`;

/**
 * Answer an evidence question using pre-fetched historical data.
 *
 * The caller passes the gathered evidence in; the function returns both the
 * AI-generated answer and the evidence so the route gets a complete response
 * in one shot (no reassembly needed).
 *
 * Supports multi-turn conversations via the optional `history` array.
 * Each history entry is { query, answer } from a previous turn.
 *
 * @param {Object} params
 * @param {string} params.query - The user's question
 * @param {Object} params.evidence - Structured data from evidence gatherers
 * @param {string} params.intent - Detected intent category
 * @param {Array} [params.history] - Previous conversation turns [{ query, answer }]
 * @returns {Promise<{ answer: string, evidence: Object }>}
 */
export async function answerEvidenceQuestion({ query, evidence, intent, history = [] }) {
  const messages = [];
  ...
  // Current turn with fresh evidence
  messages.push({
    role: 'user',
    content: JSON.stringify({ question: query, intent, evidence }, null, 0),
  });
...
// ---------------------------------------------------------------------------
// Rate lede — 1-2 sentence plain-English explanation for rec_v2 rows
// ---------------------------------------------------------------------------

/**
 * Resolve the Anthropic client. Tests may set `globalThis.__TEST_ANTHROPIC_CLIENT`
 * to override the real SDK instance; production code reuses the module-level
 * client constructed at the top of this file.
 */
function getAnthropicClient() {
  if (globalThis.__TEST_ANTHROPIC_CLIENT) return globalThis.__TEST_ANTHROPIC_CLIENT;
  return client;
}

function buildDeterministicLede({ rec, comps }) {
  const signal = rec.demand_signal || 'mixed';
  const pct = comps?.pct ?? 0;
  const median = rec.tcpn_2n ? `$${Math.round(rec.tcpn_2n)}` : 'the market';
  const day = rec.day_type === 'weekend' ? 'weekend night' : 'weeknight';
  if (signal === 'tight') return `${day} is running tight — ${pct}% of comps already booked, median sitting at ${median}.`;
  if (signal === 'open')  return `${day} is soft — only ${pct}% of comps booked, market median at ${median}.`;
  return `${day} is mixed — ${pct}% of comps booked, median ${median}.`;
}

/**
 * Generate a 1-2 sentence plain-English explanation of a nightly rate recommendation.
 * Falls back to a deterministic template if the model call fails.
 * @param {{ rec: Object, comps: { total:number, booked:number, pct:number } }} args
 * @returns {Promise<string>}
 */
export async function generateRateLede({ rec, comps }) {
  const prompt = [
    `Date: ${rec.check_date} (${rec.day_type || 'n/a'})`,
    `Recommended nightly rate: $${rec.rec_nightly_rate}`,
    `Decision: ${rec.verdict}`,
    `Demand signal: ${rec.demand_signal}`,
    `Market median (2n): $${rec.tcpn_2n}`,
    `Comps: ${comps.booked}/${comps.total} booked (${comps.pct}%)`,
    `Confidence: ${rec.confidence}/100`,
    '',
    'In 1-2 sentences, explain this recommendation in plain English to a host. Lead with the demand situation. Do not recite numbers verbatim — narrate them. Use strong language for tight demand, soft for open. No preamble, no hedging.',
  ].join('\n');
  ...
  try {
    ...
    // Observability: deterministic fallback is silent otherwise and would get
    // cached into recommendations_v2.lede_text indistinguishable from Claude output.
    console.error(`[generateRateLede] falling back to deterministic template: ${e?.message || e}`);
  }
  return buildDeterministicLede({ rec, comps });
}
