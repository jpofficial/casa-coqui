...
export async function notifyStaff({ staffIds, title, body, type = 'staff', data = {}, localizer }) {
  try {
    if (!staffIds || staffIds.length === 0) {
      console.warn(`[notifyStaff] Called with 0 recipients — type=${type}`);
    ...
    const results = await Promise.all(
      staffIds.map(async (uid) => {
        ...
        // Check notification preferences — skip push if category is muted
        const TYPE_TO_PREF_KEY = {
          maintenance: 'maintenance',
          cleaning_assignment: 'cleaning',
          cleaning_update: 'cleaning',
        ...
        if (prefKey) {
          try {
            ...
            if (prefDoc.exists && prefDoc.data()[prefKey] === false) {
              pushMuted = true;
            }
          } catch (prefErr) {
            console.warn(`[notifyStaff] Pref lookup failed for ${uid}:`, prefErr.message);
          }
        }
        ...
        if (!pushMuted) {
          try {
            ...
            if (!tokenSnap.empty) {
              // Send to ALL tokens in parallel — user may have multiple devices,
              // and FCM can accept stale tokens without error (delivery fails silently).
              const pushData = { type, ...data, title: String(pushTitle), body: String(pushBody) };
              const sendResults = await Promise.all(
                tokenSnap.docs.map(async (tokenDocSnap) => {
                  const tkn = tokenDocSnap.data().token;
                  if (!tkn) return false;
                  try {
                    await messaging.send({ token: tkn, data: pushData });
                    return true;
                  } catch (fcmErr) {
                    console.error(`[notifyStaff] FCM failed for ${uid} token=${tkn.slice(0, 12)}...:`, fcmErr.message);
        ...
        console.log(`[notifyStaff] uid=${uid} muted=${pushMuted} tokens=${tokenCount} pushSent=${pushSent} method=${method}`);
        return { pushSent };
      })
    );
...
export async function notifyAdminAndCohost({ title, body, type = 'staff', data = {}, localizer }) {
  try {
    ...
    console.log(`[notifyAdminAndCohost] type=${type} recipients=${staffIds.length} ids=${staffIds.join(',')}`);
    if (staffIds.length === 0) {
      console.warn('[notifyAdminAndCohost] No active admin/cohost users found — notification will not be sent');
    }
