...
// v2 namespace contract — composite-key threading (2026-05-11)
...
test('isUnmatchedKey: v2 booking:<id> is MATCHED', () => {
  ...
  assert.strictEqual(isUnmatchedKey('booking:HMABC'), false);
});

test('isUnmatchedKey: v2 guest:<hash> is UNMATCHED', () => {
  assert.strictEqual(isUnmatchedKey('guest:abc123'), true);
  assert.strictEqual(isUnmatchedKey('guest:0123456789abcdef'), true);
});
