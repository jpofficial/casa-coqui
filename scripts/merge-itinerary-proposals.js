 ...
 *
 * Usage:
 *   node scripts/merge-itinerary-proposals.js          # dry-run, prints plan
 *   node scripts/merge-itinerary-proposals.js --apply  # actually merge
 *   node scripts/merge-itinerary-proposals.js --apply --seed
...
const PROPOSED_DIR = path.join(REPO_ROOT, 'tasks/itinerary-research/proposed');
const ARCHIVED_DIR = path.join(PROPOSED_DIR, 'archived');
const SEED_DIR = path.join(REPO_ROOT, 'tasks/itinerary-research/seed-final');
...
function main() {
  ...
  if (SEED) {
    log('\nRe-seeding DynamoDB via scripts/seed-activities.js...');
    execSync('node scripts/seed-activities.js', { cwd: REPO_ROOT, stdio: 'inherit' });
  } else {
    log('\nDone. Re-seed DynamoDB with: node scripts/seed-activities.js');
  }
}
